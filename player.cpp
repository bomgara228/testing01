...
Player::Player() {
	...
	shild.loadFromFile(IMAGES_FOLDER + PLAYER_SHIELD);
	sprite.setTexture(texture);	
	shield.setTexture(shild);
	sprite.setScale(0.7f, 0.7f); shield.setScale(0.4f, 0.4f);
	texture.setSmooth(true);
	shild.setSmooth(true);
...
void Player::update() {
	...
	sf::FloatRect g_bounds = sprite.getGlobalBounds();
	sf::Vector2f pos = shield.getPosition();
	sf::FloatRect h_bounds = shield.getGlobalBounds();
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::S) &&
		...
		sprite.move(0.f, PLAYER_SPEED_Y );
	if (hp <= 0) { shield.setPosition(sprite.getPosition()); }
}
void Player::draw(sf::RenderWindow& window) { window.draw(sprite); window.draw(shield);}
